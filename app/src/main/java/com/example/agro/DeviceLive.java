        ...
        mDatabase.child("users").child(CustomUtils.loggedUser.getUid()).child("devices").child(deviceId).child("live").addValueEventListener(new ValueEventListener() {
            ...
            public void onDataChange(@NonNull DataSnapshot snapshot) {
               if(snapshot.getValue()!=null){
                   ...
                   liveViewTempratureGauge.setValue(getPrecentage(125, (float)stat.getTs()));
                   liveViewHumidityGauge.setValue(getPrecentage(100, (float)stat.getHs()));
                   liveViewMoistureGauge.setValue(getPrecentage(100, (float)stat.getSms()));
                   liveViewSmokeGauge.setValue(getPrecentageBool( (float)stat.getGss()));
                   liveViewFalmeGauge.setValue(getPrecentageBool((float)stat.getFs()));
               }
